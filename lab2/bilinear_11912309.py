...
def bilinear_11912309(input_file: str, dim, output_file: str = 'bilinear_test.tif') -> np.ndarray:
    ...
    #                                    (int(raw_pic[int(target_x[x]), yc]) - int(raw_pic[int(target_x[x]), yf]))
    #     else:
    #         xf = int(target_x[x])
    #         xc = xf + 1
    #         for y in range(target_col):
    ...
    #                                    (int(raw_pic[xc, int(target_y[y])]) - int(raw_pic[xf, int(target_y[y])]))
    #             else:
    #                 yf = int(y)
    #                 yc = yf + 1
    #
    #                 xl = (target_x[x] - xf) * (int(raw_pic[xc, yf]) - int(raw_pic[xf, yf]))
    #                 xr = (target_x[x] - xf) * (int(raw_pic[xc, yc]) - int(raw_pic[xf, yc]))
    #
    ...
    for x in range(target_row):
        ...
        for y in range(target_col):
            ...
            vl = raw_pic[xf, yf] + (target_x[x] - xf) * (int(raw_pic[xc, yf]) - int(raw_pic[xf, yf]))
            vr = raw_pic[xf, yc] + (target_x[x] - xf) * (int(raw_pic[xc, yc]) - int(raw_pic[xf, yc]))
...
bilinear_11912309(raw_file, dim_shrank, "shrank_bilinear_11912309.png")
bilinear_11912309(raw_file, dim_shrank, "shrank_bilinear_11912309.tif")
